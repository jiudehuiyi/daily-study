...
一个经常被问到的问题，就是**为什么不把组件的数据直接存放在组件类的成员变量中？**比如像下面这样：
...
React 非常巧妙地用**任务队列**解决了这个问题，可以理解为每次 setState 函数调用都会往 React 的任务队列里放一个任务，多次 setState 调用自然会往队列里放多个任务。React 会选择时机去批量处理队列里执行任务，当批量处理开始时，React 会合并多个 setState 的操作，比如上面的三个 setState 就被合并为只更新 state 一次，也只引发一次重新渲染。
